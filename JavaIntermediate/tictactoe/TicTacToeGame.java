...
public class TicTacToeGame
{
    ...
    public boolean isDraw() 
    {
        ...
        for (int y=0; y<3; y++) {
            for (int x=0; x < 3; x++) {
        if (getGameWinner() == ' ' && getPieceAt(x,y) != ' '){
               gridFilled += 1;
            }
            
        }
    }
    if (gridFilled == 9 && getGameWinner() == ' ') {
        return true;
    }
        else {
    ...
    public char getGameWinner() 
    {
        ...
        for (int y=0; y<3; y++) {
            if (getPieceAt(0,y) == getPieceAt(1,y) && (getPieceAt(0,y) ==getPieceAt(2,y)) ){
                return getPieceAt(0,y);
            }
            
    }
    for (int x=0; x < 3; x++) {
        if (getPieceAt(x,0) == getPieceAt(x,1) && (getPieceAt(x,0) ==getPieceAt(x,2)) ){
                return getPieceAt(x,0);
            }
        }       
    
        if (getPieceAt(0,0) == getPieceAt(1,1) && (getPieceAt(0,0) == getPieceAt(2,2))) {
        return getPieceAt(0,0);
    }
        if (getPieceAt(2,0) == getPieceAt(1,1) && (getPieceAt(2,0) == getPieceAt(0,2))) {
        return getPieceAt(0,0);
    }
